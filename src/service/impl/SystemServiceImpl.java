...
public class SystemServiceImpl extends BaseService implements ISystemService {
    ...
    public BaseArgument select(BaseArgument arg) {
        try {
            System entity = (System) arg.getObj();
            List<System> dataList = dao.select(getMapper(entity, "select"), entity);
            if(dataList != null){
    ...
    public BaseArgument selectPaging(BaseArgument arg) {
        try {
            ...
            List<System> dataList = dao.select(getMapper(entity, "select"), entity, new RowBounds(page.getStartRow(), page.getPageSize()));
            if(dataList != null){
                arg.success().setToRtn("list", dataList);
            } else {
                arg.fail();
